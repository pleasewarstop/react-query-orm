некоторые массивы могут содержать больше информации, чем другие
max-length

Идём по старому апи
Копируем его поля
Если поле - сущность, то в копию ставим её и идём дальше
В копии должны быть старые внутренние сущности
Прежде, чем идём во внутреннюю сущность, смотрим, не изменилась ли она
Если изменилась, то может быть hardApply('vm', { ... })
У нас в копии уже новые сущности и может быть изменилась орм
Мы должны парсить для орм это как будто получили ответ апи
Может возникнуть орм конфликт с пришедшим из апи новым значением
Возвращаемся к месту, где старое апи актуально

Считаем за потенциальное обновление всё когда пришла сущность
Но после extract если ничего не поменялось, то setState не делаем

Если орм-id изменился, инвалидация апи
(но может хватать всех полей, hasAllFields('host', { apiKey: (inst, path) => {...} })
для случая инвалидации hardApply('vm', { oki: (inst, path) => {...} })
